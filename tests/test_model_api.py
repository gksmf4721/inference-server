...
from backend.api.inference.model.model_api import inference_status
from main import app

client = TestClient(app)

# ONNX 이미지 예측 API 테스트
@patch('backend.config.onnx_conn.onnx_predict_image')
def test_predict_onnx(predict_image_mock):
    # 모킹된 함수가 반환할 가짜 결과 설정
    predict_image_mock.return_value = "fake_onnx_prediction_result"
    ...
    # 원본 이미지가 존재하면 백업 이미지 생성
    shutil.copy(original_image_path, backup_image_path)

    try:
        # ONNX 예측 API 호출
        response = client.post("/onnx/predict", json={"image_name": image_name})
        ...
        # 존재하지 않는 이미지 경로 테스트
        response = client.post("/onnx/predict", json={"image_name": "non_existent_image.jpg"})
...
# TFLite 이미지 예측 API 테스트
@patch('backend.config.tflite_conn.tflite_predict_image')
def test_predict_tflite(predict_image_mock):
    # 모킹된 함수가 반환할 가짜 결과 설정
    predict_image_mock.return_value = "fake_tflite_prediction_result"
    ...
    try:
        # TFLite 예측 API 호출
        response = client.post("/tflite/predict", json={"image_name": image_name})
        assert response.status_code == 202
        response_json = response.json()
        assert "request_id" in response_json
        assert response_json["message"] == "Request accepted and processing"
        ...
        # 존재하지 않는 이미지 경로 테스트
        response = client.post("/tflite/predict", json={"image_name": "non_existent_image.jpg"})
        assert response.status_code == 404
        assert response.json() == {"detail": "Image not found"}
    finally:
        # 테스트 후 원본 파일 덮어쓰기
...
# 추론 내역 조회 API 테스트
@patch('backend.config.db_conn.select')
...
# 추론 내역 삭제 API 테스트
@patch('backend.config.db_conn.delete')
